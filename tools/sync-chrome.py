...
# The diet and time collections, generated by tools/build-collections.py.
# In the nav for the same reason the cuisines are: they are the other axis
# people arrive on. Someone looking for vegan Indian food is not looking for a
# region, and until these pages existed the site had nothing to offer them
# except a filter panel that search engines cannot see.
COLLECTIONS = [
    ("healthy-indian-recipes.html",      "Healthy"),
    ("quick-indian-recipes.html",        "Quick (30 min)"),
    ("vegetarian-indian-recipes.html",   "Vegetarian"),
    ("vegan-indian-recipes.html",        "Vegan"),
    ("gluten-free-indian-recipes.html",  "Gluten-free"),
    ("dairy-free-indian-recipes.html",   "Dairy-free"),
    ("no-onion-no-garlic-recipes.html",  "No onion or garlic"),
    ("indian-soup-recipes.html",         "Soups"),
]
...
def nav_html(current):
    ...
    # Same shape as the cuisine menu: closed by default, but every link is in
    # the page on every page, so each collection is crawlable from anywhere.
    coll_hrefs = {h for h, _ in COLLECTIONS}
    diet = ['        <li class="nav-cuisines">',
            '          <button type="button" class="nav-cuisines-toggle%s" '
            'aria-expanded="false" aria-controls="nav-diet-menu">Collections</button>'
            % (" active" if current in coll_hrefs else ""),
            '          <ul class="nav-dropdown" id="nav-diet-menu">']
    for href, label in COLLECTIONS:
        cls = ' class="active"' if href == current else ""
        diet.append('            <li><a href="%s"%s>%s</a></li>' % (href, cls, label))
    diet += ['          </ul>', '        </li>']
    rows += diet
